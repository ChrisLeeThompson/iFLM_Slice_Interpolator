...
.
A PySide6/QML desktop UI that filters and z-interpolates image stacks
produced by Thermo Scientific iFLM software. Run this file to launch.
The selected TFS XML file is parsed, the referenced images are processed
and interpolated, and the output -- a per-stack folder inside
processed_images plus a new TFS XML file that loads in TFS Maps -- is
written next to the original.
Runs unmodified on a Thermo Scientific AutoScript (>= 4.13) Python
environment; on any other Python, install the packages in
requirements.txt. Application logic lives in the ``python_resources``
package -- this module only wires up the Qt application, the QML
engine, and the controller.
.
Authors: Chris Thompson (GitHub: ChrisLeeThompson) and Anthropic's Claude
.
If you have any questions, comments, or suggestions, please contact
Chris Thompson (GitHub: ChrisLeeThompson).
.
Thank you,
Chris Thompson
.
...
class MainOperator(QObject):
    ...
    def _get_tfs_file_valid(self):
        """True if the selected TFS XML file exists and has an .xml extension."""
    ...
    def _get_processed_data_exists(self):
        """True if the stack's processed images folder or interpolated TFS file exists."""
    ...
    def _get_processing_running(self):
        """True while a processing run is active."""
    ...
    def _set_image_filter_method_index(self, index: int):
        """Update the image filter method index."""
    ...
    def _set_gaussian_background_sigma(self, sigma: int):
        """Update the Gaussian background sigma."""
    ...
    def _set_global_background_normalization(self, enabled: bool):
        """Update the global background normalization flag."""
    ...
    def _set_unsharp_gaussian_sigma(self, sigma: float):
        """Update the unsharp Gaussian sigma."""
    ...
    def _set_interpolation_method_index(self, index: int):
        """Update the interpolation method index."""
    ...
    def delete_processed_data(self):
        ...
        if processed_dir.exists():
            try:
                ...
                logger.error("Failed to delete processed images directory.", exc_info=True)
        ...
        if interpolated_tfs_file and interpolated_tfs_file.exists():
            try:
                ...
                logger.error("Failed to delete interpolated TFS file.", exc_info=True)
    ...
    def start_processing(self):
        """Start a processing run on a background thread when the user clicks Start."""
        ...
        try:
            ...
            self._update_operator_to_statusGB("Error: cannot parse TFS file (see log)")
    ...
    def stop_processing(self):
        """Request a stop of the running worker when the user clicks Stop."""
        if self._worker:
    ...
    def _validate_channels(self, tfs_parser: TFSFileParser, interp_method) -> str | None:
        ...
        for channel in tfs_parser.get_channels():
            ...
            if len(images) < min_slices:
                noun = "slice" if len(images) == 1 else "slices"
                return (f"Error: channel '{channel.name}' has {len(images)} {noun}; "
    ...
    def _clear_worker_refs(self, thread: QThread):
        """Drop worker/thread references once that run's thread has finished."""
    ...
    def _check_processed_data_exists(self):
        """Refresh processed_data_exists_property from what is on disk."""
        if not self._tfs_file_path:
    ...
    def _update_operator_to_statusGB(self, message: str):
        """Set the status label text and notify QML."""
        self._operator_to_statusGB = message
        self.operator_to_statusGB_signal.emit(message)
    ...
    def _on_worker_stopped(self):
        """Reset the running state after a user-requested stop."""
        self._processing_running = False
        self.processing_running_signal.emit(False)
        self._update_operator_to_progressBar_visible(False)
        self._check_processed_data_exists()
    ...
    def _on_worker_error(self, message: str):
        """Show the error in the status label and reset the running state."""
        self._processing_running = False
        self.processing_running_signal.emit(False)
        self._update_operator_to_progressBar_visible(False)
...
def setup_logging():
    ...
    Logs at INFO to the console; DEBUG would emit per-slice output from
    every module and from third-party libraries.
    """
    logging.basicConfig(
